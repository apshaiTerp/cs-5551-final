 ...
 *
 */
public class MMDataAgentThread extends BackgroundAgentThread {
  
  /** Service access point */
  public final static String SERVICE_ROOT = "/external/mmdata";
  /** Parameter Request list for read requests */
  public final static String PARAM_LIST   = "?mmid=<gameID>";
  /** Parameter Replace Tag */
  public final static String REPLACE_TAG  = "<gameID>";

  public MMDataAgentThread() {
    super();
  }
  
  public MMDataAgentThread(long startID, long stopID) {
  ...
  private void executeSimpleTask() {
    ...
    List<Long> mmIDList = null;
    try {
      mmIDList = database.getMMIDList();
    ...
    if (gamesToSearch.size() > 0) {
      ...
      for (int pos = 0; pos < gamesToSearch.size(); pos++) {
        ...
        System.out.println ("Processing MM ID: " + curID);
        ...
        if (game != null) {
          ...
          try {
            database.insertMMPriceData(game);
          } catch (DatabaseOperationException doe) {
            doe.printStackTrace();
          } catch (ConfigurationException ce) {
            ce.printStackTrace();

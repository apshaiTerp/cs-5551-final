 ...
 *
 */
public class BatchBGGGameUpdateAgentThread extends BackgroundAgentThread {
  ...
  public final static String BATCH_PARAM  = "&source=hybrid&batch=50";

  public BatchBGGGameUpdateAgentThread() {
    super(0,0);
  }
  ...
  private void executeSimpleTask() {
    ...
    List<Long> gamesToSearch = null;
    try {
      gamesToSearch = database.getBggIDList();
    ...
    stopID = gamesToSearch.get(gamesToSearch.size() - 1) + 1;
    
    System.out.println ("We are hoping to update " + gamesToSearch.size() + " documents");
        ...
        if (games != null) {
          ...
          for (BGGGame game : games) {
            if (gamesToSearch.contains(game.getBggID())) {
              ...
              System.out.println (" > I'm updating game: " + game.getName());
              ...
              System.out.println (" + I'm inserting " + game.getName() + " because it apparently isn't already in the database");
